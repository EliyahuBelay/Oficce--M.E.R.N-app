...
const LogIn = async (request, response) => {
    //)return response.status(400).json({ message: 'Email is not exists' })
    ...
    // if (await users.exists({ email: !request.body.email }))return response.status(400).json({ message: 'Email is not exists' })
    // try {
